...
from parti.t5 import t5_encode_text, get_encoded_dim, DEFAULT_T5_NAME
...
class TransformerDecWrapper(nn.Module):
	def __init__(
		self,
		*,
		num_tokens,
		...
		# self.token_emb = CLIPTextEmbedder(
	...
	def forward(
		...
		**kwargs
		...
		b, num_mem, emb_frac_gradient = len(x), self.num_memory_tokens, self.emb_frac_gradient
		...
		if x.shape[1] > 0:
			...
			x = repeat(self.start_token, 'd -> b 1 d', b=1)
...
class TransformerEncWrapper(nn.Module):
	def __init__(
		self,
		*,
		num_tokens,
		...
		self.encode_texts = partial(t5_encode_text, name = 't5-large')
	...
	def forward(
		...
		**kwargs
		...
		# encode_texts = partial(t5_encode_text, name = 't5-large')
		text_token_embeds, text_mask = self.encode_texts(x ,output_device = torch.device('cuda'))
		text_mask = torch.ones(text_token_embeds.shape[:2], dtype = torch.bool)
		x, text_mask = map(lambda t: t[:, :self.max_seq_len], (text_token_embeds, text_mask))

		mask  = torch.ones(len(x), x.shape[1]).bool().to(torch.device('cuda'))
...
class XTransformer(nn.Module):
	def __init__(
		...
		**kwargs
		...
		self.encoder = TransformerEncWrapper(
			**enc_transformer_kwargs,
			attn_layers = Encoder(dim = dim, **enc_kwargs)
		)

		self.decoder = TransformerDecWrapper(
	...
	# 		enc_output = self.encoder(src, return_embeddings=True)

	# 		gen_seq = torch.empty(0, dtype=torch.long, device="cuda")
	# 		for step in range(1, 1025):
				
	# 			dec_output = self.decoder.net(gen_seq.unsqueeze(0), context=enc_output)[:,-1]
	# 			dec_output = F.softmax(dec_output, dim=1)
	# 			dec_output = torch.argmax(dec_output, dim=1) 
	# 			gen_seq = torch.cat([gen_seq, dec_output])  #  gen -> (1,1024)
				
	# 		gen_seq = gen_seq.unsqueeze(0)  
	# 		return gen_seq
	...
	def generate(self, src):
		with torch.no_grad():
			...
			for step in range(0, 1024):
				...
				# sampled = gumbel_sample(filtered_logits, temperature = 1, dim = -1)
				dec_output = torch.argmax(dec_output, dim=1) 
				# dec_output = gumbel_sample(dec_output, temperature = 1, dim = -1)
				gen_seq = torch.cat([gen_seq, dec_output])  #  gen -> (1,1024)
	...
	def forward(self, src, tgt, mask = None, attn_mask = None, src_prepend_embeds = None):
		...
		mask  = torch.ones(len(src), 128).bool().to(torch.device('cuda'))
