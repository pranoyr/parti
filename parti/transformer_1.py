	...
	def __init__(
	  self,
		*,
		...
		self.image_token_embed = nn.Embedding(8192, dim)
		...
		self.to_logits = nn.Linear(dim, 8192, bias = False)
	...
	def generate(
		...
		*,
		...
		for _ in range(image_seq_len):
			...
			dec_output = F.softmax(logits, dim=1)
			sampled = torch.argmax(dec_output, dim=1) 
