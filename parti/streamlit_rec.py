...
def reconstruction(img=None, model_name='vit-s-vqgan', titles=['origin', 'reconstruct'], checkpoint_path=None, scale=0.8):
    w, h = 256, 256
    ...
    model.eval()
    with torch.no_grad():
        z, _, encoding_indices = model.encode(img.unsqueeze(0))
